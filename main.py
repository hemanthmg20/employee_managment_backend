...
from schemas import emp_schema, emp_resopnse, ResetPasswordRequest, emp_patch_schema, emp_put_schema, emp_delete_schema,emp_verify_schema,feedback_schema
...
def get_employees(db: Session = Depends(get_db)
                #    current_user: empData = Depends(get_current_user)
                ):
    try:
        emps = db.query(empData).all()
        if not emps:
            return{"detail":"No employees were found"}
        return emps
...
def update_passkey(id: int,
    ...
    try:
        ...
        #     raise HTTPException(status_code=450, detail="New password cannot be the same as the old password")
...
@app.post('/emp/feedback',tags=['Employees'])
def emp_feedback (request : feedback_schema,
                db: Session = Depends(get_db)):
    emp = db.query(empData).filter(empData.emp_id == request.emp_id).first()
    emp.feedback = request.feedback
    db.commit()
    db.refresh(emp)
    return {"detail":" Feedback submitted successfully!"}


@app.post('/emp/verify', tags=["Authenticate user"],response_model=emp_resopnse, include_in_schema=False)
def verify_emp(request: emp_verify_schema,
                db: Session = Depends(get_db),
                # current_user: empData = Depends(get_current_user)
            ):
    try:
